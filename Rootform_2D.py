...
class Lat2d:
    ...
    def lat_plot(self, save_file, r = 2, type = 'Points', unit_cell = False, vectors = True, obsb = False):
        latpoints = []
        unit_cell_x = [0, self.x[0], self.x[0] + self.y[0], self.y[0]]
        unit_cell_y = [0, self.x[1], self.x[1] + self.y[1], self.y[1]]
        ...
        if type == 'Points':
            ...
            if vectors:
                ...
                plt.plot(unit_cell_x, unit_cell_y,linewidth = 2, color="green", zorder = 2)
            if obsb:
                plt.arrow(0, 0, self.ob[0], self.ob[1], width=0.01, color='red', length_includes_head=True, zorder=2)
            if unit_cell:
                plt.fill(unit_cell_x, unit_cell_y, color = 'yellow', zorder = 1)
            plt.box(False)
    ...
    def rf_grpchir(self, pgroup = 2, dtype = 0):
        if pgroup == 2:
            ...
            if dtype == 0:
                ...
                return np.sqrt((self.r_12)**2 + 0.25*(self.r_02 - self.r_01)**2)
